...
write_healthcheck_env_file() {
  # Persist CONFIGURATION the timer run needs. Detection results (PUBLIC_IP,
  # PUBLIC_IP_FAMILY, derived hosts) are intentionally NOT persisted so each run
  # re-detects the current public IP. LISTEN_ADDRESS and PUBLIC_IP are persisted
  # only when the user explicitly pinned them; otherwise they re-derive.
  # Reality fallback candidates also stay in the defaults (not frozen here).
  local -a kv=(
    PORT "${PORT}"
    NODE_NAME "${NODE_NAME}"
    NODE_ROLE "${NODE_ROLE}"
